...
namespace plume {
    ...
    MetalShader::MetalShader(const MetalDevice *device, const void *data, uint64_t size, const char *entryPointName, const RenderShaderFormat format) {
        ...
        this->format = format;
        this->functionName = ((entryPointName != nullptr) ? NS::String::string(entryPointName, NS::UTF8StringEncoding) : MTLSTR(""))->retain();
    ...
    void MetalShader::setName(const std::string &name) {
        ...
        debugName = NS::String::string(name.c_str(), NS::UTF8StringEncoding)->retain();
    ...
    void MetalCommandList::begin() {
        ...
        // commandBufferWithUnretainedReferences() returns an autoreleased
        // command buffer. Keep an owned reference until commit() balances it.
        mtl->retain();

...
void graphics_config_to_json(nlohmann::json& j, const ultramodern::renderer::GraphicsConfig& config) {
    j = nlohmann::json{
        {"_config_version", graphics_config_version},
...
void graphics_config_from_json(const nlohmann::json& j, ultramodern::renderer::GraphicsConfig& config) {
    ...
    // The Deck's 1280x800 display does not benefit enough from automatic
    // multi-sampling and a high-precision framebuffer to justify their cost.
    // Migrate existing Deck installs once so the optimized preset also takes
    // effect for users who already selected a ROM with an older build.
    ...
    // DKR reuses dynamic racer display-list state in ways that do not yet have
    // stable identities across RT64-generated frames. Migrate the Deck preset
    // back from interpolation so existing installs do not flash duplicate or
    // displaced racers between the game's original frames.
    if (is_steam_deck && config_version < 5) {
...
std::filesystem::path get_app_folder_path() {
   ...
   if (homedir != nullptr) {
       recomp_dir = std::filesystem::path{homedir} / "Library/Application Support" / std::u8string{program_id};
   }
#endif
...
void reset_graphics_options() {
    ...
    new_config.res_option = is_steam_deck ? ultramodern::renderer::Resolution::Original2x : res_default;
    ...
    new_config.msaa_option = is_steam_deck ? ultramodern::renderer::Antialiasing::None : msaa_default;
    new_config.rr_option = rr_default;
    new_config.hpfb_option = is_steam_deck ? ultramodern::renderer::HighPrecisionFramebuffer::Off : hpfb_default;
...
bool load_graphics_config(const std::filesystem::path& path) {
    ...
    const int config_version = from_or_default(config_json, "_config_version", 1);
    ultramodern::renderer::GraphicsConfig new_config{};
    graphics_config_from_json(config_json, new_config);
    ultramodern::renderer::set_graphics_config(new_config);

    // Persist Deck preset migrations immediately. A renderer failure during
    // shutdown should not leave the old interpolation setting on disk.
    if (is_steam_deck && config_version < graphics_config_version) {
        save_graphics_config(path);
    }
...
bool load_controls_config(const std::filesystem::path& path) {
    ...
    if (config_version < controls_config_version) {
        ...
        dino::input::InputField& secondary = dino::input::get_input_binding(
            dino::input::GameInput::B, 1, dino::input::InputDevice::Keyboard);

        // Preserve customized B bindings. Only add X when the profile still
        // has the original Left Shift binding and an unused secondary slot.
        if (defaults.size() >= 2 &&
            primary.input_type == defaults[0].input_type && primary.input_id == defaults[0].input_id &&
            secondary.input_type == 0 && secondary.input_id == 0) {
            secondary = defaults[1];
        }

        // Early Deck builds could retain controller bindings created while
        // the app was being tested with keyboard-emulating Desktop layouts.
